...
bool cliOpen(uint8_t ch, uint32_t baud)
{
  ...
  cli_node.is_open = uartOpen(ch, baud, uartbuffer, CLI_BUFFER_SIZE);
...
bool cliOpenLog(uint8_t ch, uint32_t baud)
{
  ...
  ret = uartOpen(ch, baud,uartbuffer,CLI_BUFFER_SIZE);
...
bool cliMain(void)
{
  ...
  if (len > 0 && len < sizeof(tmp_buffer)) {
      for (int i = 0; i < len; i++) {
          tmp_buffer[i] = uartRead(_DEF_UART1_DBG);
      }
      tmp_buffer[len] = '\0';          // ★ 꼭 필요
...
uint32_t cliWriteOtherUart(uint8_t ch, uint8_t *p_data, uint32_t length)
{
  return uartWrite(ch, p_data, length);
}
